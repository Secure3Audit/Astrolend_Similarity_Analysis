...
use astrolend::{
    ...
    state::astrolend_group::{BankConfig, BankConfigOpt, BankOperationalState},
};
use pretty_assertions::assert_eq;
use solana_program_test::*;

#[tokio::test]
async fn astrolend_group_bank_paused_should_error() -> anyhow::Result<()> {
    ...
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), AstrolendError::BankPaused);

    Ok(())
}

#[tokio::test]
async fn astrolend_group_bank_reduce_only_withdraw_success() -> anyhow::Result<()> {
    let test_f = TestFixture::new(Some(TestSettings {
        banks: vec![TestBankSetting {
            mint: BankMint::Usdc,
            config: None,
...
#[tokio::test]
async fn astrolend_group_bank_reduce_only_deposit_success() -> anyhow::Result<()> {
    let test_f = TestFixture::new(Some(TestSettings {
        banks: vec![
            TestBankSetting {
                mint: BankMint::Usdc,
...
#[tokio::test]
async fn astrolend_group_bank_reduce_only_borrow_failure() -> anyhow::Result<()> {
    let test_f = TestFixture::new(Some(TestSettings {
        banks: vec![
            TestBankSetting {
                mint: BankMint::Usdc,
    ...
    let lender_astl_account = test_f.create_astrolend_account().await;
    ...
    let borrower_astl_account = test_f.create_astrolend_account().await;
    let borrower_token_account_usdc = test_f
        .usdc_mint
        .create_token_account_and_mint_to(100_000)
        .await;
    borrower_astl_account
    ...
    let res = borrower_astl_account
        .try_bank_borrow(borrower_token_account_sol.key, sol_bank_f, 1)
        .await;

    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), AstrolendError::BankReduceOnly);

    Ok(())
}

#[tokio::test]
async fn astrolend_group_bank_reduce_only_deposit_failure() -> anyhow::Result<()> {
    let test_f = TestFixture::new(Some(TestSettings {
        banks: vec![TestBankSetting {
            mint: BankMint::Usdc,
            config: None,
