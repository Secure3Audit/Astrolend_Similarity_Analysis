...
use astrolend::{
    ...
    state::{
        astrolend_account::DISABLED_FLAG,
        astrolend_group::{BankConfig, BankConfigOpt, BankVaultType, GroupConfig},
    },
};
...
#[tokio::test]
async fn astrolend_group_handle_bankruptcy_unauthorized() -> anyhow::Result<()> {
    ...
    let borrower_account = test_f.create_astrolend_account().await;
    ...
    let mut borrower_astl_account = borrower_account.load().await;
    borrower_astl_account.lending_account.balances[0]
        .asset_shares
        .value = 0_i128.to_le_bytes();
    borrower_account.set_account(&borrower_astl_account).await?;
    ...
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), AstrolendError::Unauthorized);

    Ok(())
}

#[tokio::test]
async fn astrolend_group_handle_bankruptcy_perimssionless() -> anyhow::Result<()> {
    let mut test_f = TestFixture::new(Some(TestSettings {
        group_config: Some(GroupConfig { admin: None }),
        banks: vec![
            TestBankSetting {
    ...
    let borrower_astrolend_account = borrower_account.load().await;
    assert!(borrower_astrolend_account.get_flag(DISABLED_FLAG));
    assert_eq!(
        borrower_astrolend_account.lending_account.balances[1].liability_shares,
        I80F48!(0.0).into()
    );
