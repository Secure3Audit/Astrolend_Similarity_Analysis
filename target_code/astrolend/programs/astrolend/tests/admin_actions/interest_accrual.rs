...
use astrolend::{
    prelude::GroupConfig,
    state::astrolend_group::{Bank, BankConfig, BankVaultType, InterestRateConfig},
};
use pretty_assertions::assert_eq;
use solana_program_test::*;

#[tokio::test]
async fn astrolend_group_accrue_interest_rates_success_1() -> anyhow::Result<()> {
    ...
    let borrower_bank_account = borrower_astl_account.lending_account.balances[1];
    ...
    let lender_bank_account = lender_astl_account.lending_account.balances[0];
...
#[tokio::test]
async fn astrolend_group_accrue_interest_rates_success_2() -> anyhow::Result<()> {
    let test_f = TestFixture::new(Some(TestSettings {
        banks: vec![
            TestBankSetting {
                mint: BankMint::Usdc,
    ...
    test_f.astrolend_group.try_collect_fees(usdc_bank_f).await?;
