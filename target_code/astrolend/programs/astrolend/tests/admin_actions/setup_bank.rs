...
async fn add_bank_success() -> anyhow::Result<()> {
    ...
    for (mint_f, bank_config) in mints {
        ...
        let _ = {
            ...
            assert_eq!(group, test_f.astrolend_group.key);
...
#[tokio::test]
async fn astrolend_group_add_bank_failure_inexistent_pyth_feed() -> anyhow::Result<()> {
    // Setup test executor with non-admin payer
    let test_f = TestFixture::new(None).await;
    ...
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), AstrolendError::InvalidOracleAccount);
...
async fn configure_bank_success(bank_mint: BankMint) -> anyhow::Result<()> {
    ...
    let config_bank_opt = BankConfigOpt {
        interest_rate_config: Some(astrolend::state::astrolend_group::InterestRateConfigOpt {
