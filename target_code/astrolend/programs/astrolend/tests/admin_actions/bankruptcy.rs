...
use astrolend::{
    prelude::{GroupConfig, AstrolendError},
    state::astrolend_group::{BankConfig, BankVaultType},
};
...
#[tokio::test]
async fn astrolend_group_handle_bankruptcy_failure_not_bankrupt(
    ...
    let lp_astl_account_f = test_f.create_astrolend_account().await;
    ...
    let user_astl_account_f = test_f.create_astrolend_account().await;
    ...
    let res = test_f
        .astrolend_group
        .try_handle_bankruptcy(debt_bank_f, &user_astl_account_f)
        .await;

    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), AstrolendError::AccountNotBankrupt);
...
#[test_case(BankMint::T22WithFee, BankMint::Sol)]
#[tokio::test]
async fn astrolend_group_handle_bankruptcy_failure_no_debt(
    collateral_mint: BankMint,
    debt_mint: BankMint,
) -> anyhow::Result<()> {
    // -------------------------------------------------------------------------
    ...
    let mut user_astl_account = user_astl_account_f.load().await;
    user_astl_account.lending_account.balances[0]
        .asset_shares
        .value = 0_i128.to_le_bytes();
    user_astl_account_f.set_account(&user_astl_account).await?;

    let res = test_f
        .astrolend_group
        .try_handle_bankruptcy(collateral_bank_f, &user_astl_account_f)
        .await;

    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), AstrolendError::BalanceNotBadDebt);
...
#[test_case(BankMint::T22WithFee, BankMint::Sol)]
#[tokio::test]
async fn astrolend_group_handle_bankruptcy_success(
    collateral_mint: BankMint,
    debt_mint: BankMint,
) -> anyhow::Result<()> {
    // -------------------------------------------------------------------------
...
#[tokio::test]
async fn astrolend_group_handle_bankruptcy_success_fully_insured(
    borrow_amount: f64,
    collateral_mint: BankMint,
    debt_mint: BankMint,
) -> anyhow::Result<()> {
    ...
    // Artificially nullify the collateral to place the account in a bankrupt state
    let mut user_astl_account = user_astl_account_f.load().await;
    user_astl_account.lending_account.balances[0].asset_shares = I80F48::ZERO.into();
    user_astl_account_f.set_account(&user_astl_account).await?;
    ...
    test_f
        .astrolend_group
        .try_handle_bankruptcy(test_f.get_bank(&debt_mint), &user_astl_account_f)
        .await?;
    ...
    let user_astl_account = user_astl_account_f.load().await;
    let user_collateral_balance = user_astl_account.lending_account.balances[1];
    ...
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), AstrolendError::AccountDisabled);
...
#[tokio::test]
async fn astrolend_group_handle_bankruptcy_success_partially_insured(
    borrow_amount: f64,
    ...
    let (pre_lp_collateral_amount, pre_user_debt_amount, pre_liquidity_vault_balance) = (
        collateral_bank.get_liability_amount(
            lp_astl_account_f.load().await.lending_account.balances[0]
                .asset_shares
                .into(),
        )?,
        debt_bank.get_liability_amount(
            user_astl_account_f.load().await.lending_account.balances[1]
                .liability_shares
                .into(),
        )?,
        debt_bank_f
    ...
    let borrower_astl_account = user_astl_account_f.load().await;
    let (post_user_debt_balance, post_liquidity_vault_balance) = (
        borrower_astl_account.lending_account.balances[1],
        debt_bank_f
            .get_vault_token_account(BankVaultType::Liquidity)
            .await
            .balance()
...
#[test_case(10_000., BankMint::T22WithFee, BankMint::Sol)]
#[tokio::test]
async fn astrolend_group_handle_bankruptcy_success_not_insured(
    borrow_amount: f64,
    collateral_mint: BankMint,
    debt_mint: BankMint,
) -> anyhow::Result<()> {
...
#[tokio::test]
async fn astrolend_group_handle_bankruptcy_success_not_insured_3_depositors() -> anyhow::Result<()> {
    ...
    let mut borrower_astl_account = borrower_astl_account_f.load().await;
    borrower_astl_account.lending_account.balances[0]
        .asset_shares
        .value = 0_i128.to_le_bytes();

    borrower_astl_account_f
        .set_account(&borrower_astl_account)
        .await?;

    test_f
        .astrolend_group
        .try_handle_bankruptcy(usdc_bank_f, &borrower_astl_account_f)
        .await?;

    let borrower_astl_account = borrower_astl_account_f.load().await;
    let borrower_usdc_balance = borrower_astl_account.lending_account.balances[1];
    ...
    let lender_1_astl_account = lender_1_astl_account_f.load().await;
    let usdc_bank = usdc_bank_f.load().await;

    let lender_usdc_value = usdc_bank.get_asset_amount(
        lender_1_astl_account.lending_account.balances[0]
            .asset_shares
            .into(),
    )?;
