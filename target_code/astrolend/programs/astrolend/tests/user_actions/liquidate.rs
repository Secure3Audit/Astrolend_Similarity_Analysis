...
use astrolend::{
    prelude::*,
    state::astrolend_group::{Bank, BankConfig, BankConfigOpt, BankVaultType},
};
...
#[tokio::test]
async fn astrolend_account_liquidation_success(
    deposit_amount: f64,
    borrow_amount: f64,
    ...
    let (liquidatee_astl_account_f, borrow_amount_actual) = {
        let liquidatee_astl_account_f = test_f.create_astrolend_account().await;
        ...
        (liquidatee_astl_account_f, borrow_amount_actual)
    };

    // Liquidator

    let liquidator_astl_account_f = {
        let liquidator_astl_account_f = test_f.create_astrolend_account().await;
    ...
    let debt_mint_liquidator_balance = debt_bank.get_asset_amount(
        liquidator_astl_ma.lending_account.balances[0]
            .asset_shares
            .into(),
    )?;
    assert_eq_noise!(
    ...
    let collateral_mint_liquidatee_balance = collateral_bank
        .get_liability_amount(
            liquidatee_astl_ma.lending_account.balances[0]
                .asset_shares
                .into(),
        )
        .unwrap();
...
#[tokio::test]
async fn astrolend_account_liquidation_success_many_balances() -> anyhow::Result<()> {
    ...
    let lender_astl_account_f = test_f.create_astrolend_account().await;
    let lender_token_account_usdc = test_f
        .usdc_mint
        .create_token_account_and_mint_to(2_000)
        .await;
    lender_astl_account_f
    ...
    lender_astl_account_f
        .try_liquidate(&borrower_astl_account_f, sol_bank_f, 1, usdc_bank_f)
        .await?;
    ...
    let depositor_ma = lender_astl_account_f.load().await;
    let borrower_ma = borrower_astl_account_f.load().await;
...
#[tokio::test]
async fn astrolend_account_liquidation_success_swb() -> anyhow::Result<()> {
...
#[tokio::test]
async fn astrolend_account_liquidation_failure_liquidatee_not_unhealthy() -> anyhow::Result<()> {
    let test_f = TestFixture::new(Some(TestSettings {
        banks: vec![
            TestBankSetting {
                mint: BankMint::Usdc,
    ...
    let res = lender_astl_account_f
        .try_liquidate(&borrower_astl_account_f, sol_bank_f, 1, usdc_bank_f)
        .await;

    assert!(res.is_err());

    assert_custom_error!(res.unwrap_err(), AstrolendError::IllegalLiquidation);

    Ok(())
}

#[tokio::test]
async fn astrolend_account_liquidation_failure_liquidation_too_severe() -> anyhow::Result<()> {
    ...
    let res = lender_astl_account_f
        .try_liquidate(&borrower_astl_account_f, sol_bank_f, 10, usdc_bank_f)
        .await;
...
#[tokio::test]
async fn astrolend_account_liquidation_failure_liquidator_no_collateral() -> anyhow::Result<()> {
    let test_f = TestFixture::new(Some(TestSettings {
        banks: vec![
            TestBankSetting {
                mint: BankMint::Usdc,
    ...
    let res = lender_astl_account_f
        .try_liquidate(&borrower_astl_account_f, sol_eq_bank_f, 2, usdc_bank_f)
        .await;

    assert_custom_error!(res.unwrap_err(), AstrolendError::IllegalLiquidation);

    let res = lender_astl_account_f
        .try_liquidate(&borrower_astl_account_f, sol_eq_bank_f, 1, usdc_bank_f)
        .await;
...
#[tokio::test]
async fn astrolend_account_liquidation_failure_bank_not_liquidatable() -> anyhow::Result<()> {
    let test_f = TestFixture::new(Some(TestSettings::all_banks_payer_not_admin())).await;
    ...
    let res = lender_astl_account_f
        .try_liquidate(&borrower_astl_account_f, sol_eq_bank_f, 1, sol_bank_f)
        .await;
