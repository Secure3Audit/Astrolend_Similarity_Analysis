...
async fn automatic_interest_payments() -> anyhow::Result<()> {
    ...
    // Borrow SOL from borrower astl account
    borrower_astl_account_f
    ...
    let lender_astl_account = lender_astl_account_f.load().await;
    ...
    assert_eq_noise!(
        sol_bank
            .get_asset_amount(
                lender_astl_account.lending_account.balances[0]
                    .asset_shares
                    .into()
            )
            .unwrap(),
...
#[tokio::test]
async fn astrolend_account_correct_balance_selection_after_closing_position() -> anyhow::Result<()> {
    let test_f = TestFixture::new(Some(TestSettings::all_banks_payer_not_admin())).await;
    ...
    let mut astrolend_account = lender_astl_account_f.load().await;
...
async fn emissions_test() -> anyhow::Result<()> {
    ...
    assert_custom_error!(res.unwrap_err(), AstrolendError::EmissionsAlreadySetup);

    // Fund SOL bank
    let sol_lender_account = test_f.create_astrolend_account().await;
    ...
    let astl_account_f = test_f.create_astrolend_account().await;
...
async fn account_flags() -> anyhow::Result<()> {
    ...
    astl_account_f.try_set_flag(FLASHLOAN_ENABLED_FLAG).await?;

    let astl_account_data = astl_account_f.load().await;

    assert_eq!(astl_account_data.account_flags, FLASHLOAN_ENABLED_FLAG);

    assert!(astl_account_data.get_flag(FLASHLOAN_ENABLED_FLAG));

    astl_account_f.try_unset_flag(FLASHLOAN_ENABLED_FLAG).await?;

    let astl_account_data = astl_account_f.load().await;

    assert_eq!(astl_account_data.account_flags, 0);

    let res = astl_account_f.try_set_flag(DISABLED_FLAG).await;

    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), AstrolendError::IllegalFlag);

    let res = astl_account_f.try_unset_flag(IN_FLASHLOAN_FLAG).await;
