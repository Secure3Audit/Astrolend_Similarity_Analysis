...
async fn lending_account_close_balance() -> anyhow::Result<()> {
    ...
    let res = lender_astl_account_f.try_balance_close(sol_bank).await;

    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), AstrolendError::IllegalBalanceState);
    ...
    let res = borrower_astl_account_f.try_balance_close(sol_bank).await;
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), AstrolendError::IllegalBalanceState);
    ...
    let res = borrower_astl_account_f
        ...
        .await;
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), AstrolendError::NoLiabilityFound);

    // This issue is not that bad, because the user can still borrow other assets (isolated liab < empty threshold)
    let res = borrower_astl_account_f.try_balance_close(sol_eq_bank).await;
    assert!(res.is_ok());
