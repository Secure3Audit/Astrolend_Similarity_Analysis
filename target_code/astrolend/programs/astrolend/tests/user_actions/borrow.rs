...
use astrolend::{
    ...
    prelude::*,
    state::astrolend_group::{BankConfigOpt, BankVaultType},
};
...
#[tokio::test]
async fn astrolend_account_borrow_success(
    deposit_amount: f64,
    borrow_amount: f64,
    ...
    let post_vault_balance = debt_bank_f
        ...
        .balance()
        .await;
    let astrolend_account = user_astl_account_f.load().await;
    ...
    let active_balance_count = astrolend_account
        .lending_account
...
#[tokio::test]
async fn astrolend_account_borrow_failure_not_enough_collateral(
    deposit_amount: f64,
    ...
    let res = borrower_astl_account_f
        .try_bank_borrow(
        ...
        .await;
    assert_custom_error!(res.unwrap_err(), AstrolendError::RiskEngineInitRejected);
...
#[tokio::test]
async fn astrolend_account_borrow_failure_borrow_limit(
    borrow_cap: f64,
    borrow_amount_ok: f64,
    borrow_amount_failed: f64,
    collateral_mint: BankMint,
    ...
    assert_custom_error!(
        res.unwrap_err(),
        AstrolendError::BankLiabilityCapacityExceeded
    );
...
async fn isolated_borrows() -> anyhow::Result<()> {
    ...
    assert!(res.is_err());
    assert_custom_error!(res.unwrap_err(), AstrolendError::IsolatedAccountIllegalState);
