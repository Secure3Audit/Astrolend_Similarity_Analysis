...
use astrolend::state::astrolend_group::{BankConfigOpt, BankVaultType};
use astrolend::{assert_eq_with_tolerance, prelude::*};
...
#[tokio::test]
async fn astrolend_account_deposit_success(
    deposit_amount: f64,
    bank_mint: BankMint,
) -> anyhow::Result<()> {
    // -------------------------------------------------------------------------
    ...
    let maybe_balance = astrolend_account.lending_account.get_balance(&bank_f.key);
...
#[tokio::test]
async fn astrolend_account_deposit_failure_capacity_exceeded(
    deposit_cap: f64,
    ...
    let res = user_astl_account_f
        ...
        .await;
    assert_custom_error!(res.unwrap_err(), AstrolendError::BankAssetCapacityExceeded);
...
#[tokio::test]
async fn astrolend_account_deposit_failure_wrong_token_program() -> anyhow::Result<()> {
    // -------------------------------------------------------------------------
    // Setup
    // -------------------------------------------------------------------------
    ...
    let accounts = astrolend::accounts::LendingAccountDeposit {
        astrolend_group: astrolend_account.group,
        astrolend_account: user_astl_account_f.key,
    ...
    let deposit_ix = Instruction {
        ...
        accounts,
        data: astrolend::instruction::LendingAccountDeposit {
