...
use astrolend::{prelude::*, state::astrolend_account::FLASHLOAN_ENABLED_FLAG};
...
async fn flashloan_success_1op() -> anyhow::Result<()> {
    ...
    let borrow_ix = borrower_astl_account_f
        .make_bank_borrow_ix(borrower_token_account_f_sol.key, sol_bank, 1_000)
        .await;

    let repay_ix = borrower_astl_account_f
        .make_bank_repay_ix(
    ...
    let flash_loan_result = borrower_astl_account_f
...
async fn flashloan_fail_missing_fe_ix() -> anyhow::Result<()> {
    ...
    let start_ix = borrower_astl_account_f
    ...
    assert_custom_error!(res.unwrap_err(), AstrolendError::IllegalFlashloan);
...
async fn flashloan_fail_missing_invalid_sysvar_ixs() -> anyhow::Result<()> {
    ...
    let start_ix = Instruction {
        ...
        accounts: astrolend::accounts::LendingAccountStartFlashloan {
            astrolend_account: borrower_astl_account_f.key,
            signer: test_f.context.borrow().payer.pubkey(),
            ixs_sysvar: Pubkey::default(),
        }
        .to_account_metas(Some(true)),
        data: astrolend::instruction::LendingAccountStartFlashloan {
            end_index: ixs.len() as u64 + 1,
        }
        .data(),
    };

    let end_ix = borrower_astl_account_f
...
async fn flashloan_fail_invalid_end_fl_different_m_account() -> anyhow::Result<()> {
    ...
    let end_ix = lender_astl_account_f
        .make_lending_account_end_flashloan_ix(vec![], vec![])
        .await;
