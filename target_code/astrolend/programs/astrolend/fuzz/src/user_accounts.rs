...
pub struct UserAccount<'info> {
    pub astrol_account: AccountInfo<'info>,
    pub token_accounts: Vec<AccountInfo<'info>>,
}

impl<'info> UserAccount<'info> {
    pub fn new(
        astrol_account: AccountInfo<'info>,
    ...
    pub fn get_liquidation_banks(
        ...
        let astrolend_account_al =
            AccountLoader::<AstrolendAccount>::try_from(&self.astrol_account).ok()?;
        let astrolend_account = astrolend_account_al.load().ok()?;

        // let bank_map = get_bank_map(banks);

        let mut asset_balances = astrolend_account
            .lending_account
            .balances
            .iter()
            .filter(|blc| !blc.is_empty(astrolend::state::astrolend_account::BalanceSide::Assets))
            .collect::<Vec<_>>();
        let mut liab_balances = astrolend_account
            .lending_account
            .balances
            .iter()
            .filter(|blc| {
                !blc.is_empty(astrolend::state::astrolend_account::BalanceSide::Liabilities)
            })
            .collect::<Vec<_>>();
    ...
    pub fn get_remaining_accounts(
        ...
        let astrolend_account_al =
            AccountLoader::<AstrolendAccount>::try_from(&self.astrol_account).unwrap();
        let astrolend_account = astrolend_account_al.load().unwrap();

        let mut already_included_banks = HashSet::new();

        let mut ais = astrolend_account
            .lending_account
            .balances
            .iter()
