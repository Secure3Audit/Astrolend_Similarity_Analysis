...
use bumpalo::Bump;
use astrolend::{constants::PYTH_ID, state::astrolend_group::BankVaultType};
...
impl AccountsState {
    ...
    pub fn new_astrolend_program(&self) -> AccountInfo {
        self.new_program(astrolend::id())
    }
...
pub fn get_vault_address(bank: &Pubkey, vault_type: BankVaultType) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[vault_type.get_seed(), &bank.to_bytes()], &astrolend::ID)
}
