...
use astrolend::{assert_eq_with_tolerance, state::astrolend_group::Bank};
use astrolend_fuzz::{
    account_state::AccountsState, arbitrary_helpers::*, metrics::Metrics, AstrolendFuzzContext,
};
...
fn process_actions(ctx: FuzzerContext) -> Result<()> {
    ...
    let mut context =
        AstrolendFuzzContext::setup(&accounst_state, &ctx.initial_bank_configs, N_USERS as u8);
...
fn verify_end_state<'a>(mga: &'a AstrolendFuzzContext<'a>) -> anyhow::Result<()> {
    mga.banks.iter().try_for_each(|bank| {
        ...
        astrolend_fuzz::log!("Accounted Deposits: {}, Liabs: {}, Net {}, Outstanding Fees: {}, Net with Fees {}, Value Token Balance {}, Net Without Fees {}",
            total_deposits,
            total_liabilities,
...
fn process_action<'bump>(action: &Action, mga: &'bump AstrolendFuzzContext<'bump>) -> Result<()> {
    astrolend_fuzz::log!("==================>Action {:?}", action);
    match action {
        Action::Deposit {
            account,
            bank,
